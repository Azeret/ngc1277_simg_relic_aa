...
{The current framework reaches the lower high-Si regime but not the nominal Eftekhari-like value near $\simg\simeq0.67$ without additional yield selectivity, localized enrichment, or NIR abundance-response systematics. The ETG control sequence supports differential preservation: weak Si-rich enrichment is compatible with normal massive ETGs, while stronger preservation is reserved for NGC~1277-like relics. The reported values are chemical-evolution abundance proxies, not full spectral-index abundance inferences.}
...
\input{sections/02_baseline_model}
\input{sections/03_methods}
\input{sections/04_model_tests}
